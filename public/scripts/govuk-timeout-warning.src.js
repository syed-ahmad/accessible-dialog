(function(global, factory) {
  ...
  TimeoutWarning.prototype.startUiCountdown = function() {
    ...
    (function runTimer() {
      ...
      var text =
        'Hey We will reset your application if you do not respond in ' +
        minutesText +
        secondsText +
        '.';
      var atText =
        'Wohoo! We will reset your application if you do not respond in ' +
        atMinutesText;
      if (atSecondsText) {
        if (minutesLeft > 0) {
          atText += ' and';
  ...
  TimeoutWarning.prototype.checkIfShouldHaveTimedOut = function() {
    if (window.localStorage) {
      ...
      var seconds = Math.abs(
        (timeUserLastInteractedWithPage - new Date()) / 1000
      );
